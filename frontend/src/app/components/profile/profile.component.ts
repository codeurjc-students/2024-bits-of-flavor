...
@Component({
  ...
  export class ProfileComponent implements OnInit{
    ...
    public downloadTicket(ticket: Ticket){
        ...
        doc.setFont('helvetica', 'bold');
        doc.text('Detalles del Producto', 20, 60);
        doc.setFont('helvetica', 'normal');
        doc.text(`Producto: ${ticket.product.name}`, 20, 70);
        doc.text(`Precio: ${ticket.product.price.toFixed(2)} €`, 20, 80);
      
        doc.setFont('helvetica', 'bold');
        doc.text('Información de Recogida', 20, 100);
        doc.setFont('helvetica', 'normal');
        doc.text(`Fecha: ${ticket.date.toString()}`, 20, 110);
        doc.text('Lugar: Universidad Rey Juan Carlos (Móstoles)', 20, 120);
        doc.text('Modo de recogida: Presentar este PDF en Cafetería', 20, 130);
        ...
        doc.addImage(qrImg, 'PNG', 20, 150, 50, 50);
